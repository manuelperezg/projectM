...
<html>
			...
			function jconfirm(pregunta,mensaje,id){
                swal({
                    ...
                    buttonsStyling: true
                     ...
                     location.href = "${request.contextPath}/tipoVerificacion/remove/"+id;
                });
            }
        </script>
    </head>
    <body>
		...
		<!-- .breadcrumb-box -->
		<a href="#list-tipoVerificacion" class="skip" tabindex="-1"><g:message code="default.link.skip.label" default="Skip to content&hellip;"/></a>
					...
					<div class="input-group">
						<div class="input-group-btn search-panel">
							<button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown">
								<span id="search_concept">${session.getAttribute('tipoVerificacion_search_rows')} Registros</span> <span class="caret"></span>
							</button>
						...
						<input type="hidden" name="search_rows" id="search_rows" value="${session.getAttribute('tipoVerificacion_search_rows')}">
						<input type="hidden" name="search_clean" id="search_clean" value="0">
						<input type="text" class="form-control" name="search_word" id="search_word" placeholder="Palabra a buscar..." value="${session.getAttribute('tipoVerificacion_search_word')}">
        ...
        <div id="list-tipoVerificacion" class="content scaffold-list" role="main">
            <g:if test="${flash.message}">
                <div class="message" role="status">${flash.message}</div>
            </g:if>
            %{--
            <f:table collection="${tipoVerificacionList}" />
			 --}%

			<table>
                <thead>
                    <tr>
						<g:sortableColumn property="nombre" title="${message(code: 'tipoVerificacion.nombre.label', default: 'Nombre')}" />
                        <g:sortableColumn property="descripcion" title="${message(code: 'tipoVerificacion.descripcion.label', default: 'Descripción')}" />
                        <g:sortableColumn property="operations" title="Operaciones" style="width:100px" />
                    </tr>
                </thead>

                <tbody>
                    <g:each in="${tipoVerificacionList}" var="item" status="i">
                    ...
                    <g:if test="${tipoVerificacionCount == 0}">
						<tr>
            ...
            <div class="pagination">
                <g:paginate total="${tipoVerificacionCount ?: 0}" />
            </div>
        </div>
    </body>
</html>
