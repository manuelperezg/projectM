...
class SubsistemaController {
	...
	def showing(Subsistema subsistema) {
        respond subsistema
    }
    ...
    def create() {
		...
		if (params.maquina) {
			subsistema.maquina = Maquina.findById(params.maquina);
		}
    ...
    @Transactional
    def saving(Subsistema subsistema) {
        if (subsistema == null) {
            transactionStatus.setRollbackOnly()
            notFound()
            return
	...
	def editing(Subsistema subsistema) {
        respond subsistema
    }
    ...
    @Transactional
    def upchanging(Subsistema subsistema) {
        if (subsistema == null) {
            transactionStatus.setRollbackOnly()
            notFound()
            return
    ...
    def delete(Subsistema subsistema) {
		...
		if (Punto.findAll("FROM Punto as items WHERE items.subsistema.id = '"+subsistema.id+"'").toList().size() > 0){
			flash.message = message(code: 'default.not.deleted.message', args: [message(code: 'subsistema.label', default: 'Subsistema'), subsistema.nombre])
		}else{
    ...
    @Transactional
    def deleting(Subsistema subsistema) {
	...
	@Transactional
    def removing(Subsistema subsistema) {
