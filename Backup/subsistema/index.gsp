			...
			function jconfirm(pregunta,mensaje,id){
                swal({
                    ...
                    buttonsStyling: true
                     ...
                     location.href = "/subsistema/remove/"+id;
                });
            }
            ...
            <div class="col-xs-8" style="white-space: nowrap; float:right; padding: 0px; margin-right:3px;">
				<g:form action="index" method="POST" name="search_form">
					<div class="input-group filter-panel" style="text-align:right;width:30%;display:inline-block;top:-17px">
						<g:select optionKey="id" optionValue="nombre" name="filter_maquina" from="${maquinas}" noSelection="['ALL':'Todas las máquinas']" value="${session.getAttribute('search_filter_maquina')}"/>
					</div>
                    ...
                    <g:each in="${subsistemaList}" var="item" status="i">
                        <tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
							<td style="width:100px;">
								...
								<center>
									<a href="javascript:showImage('${item.nombre}','${item.archivo_Foto}');"><img src="${request.contextPath}/assets/mes-icons/${item.archivo_Foto}" border="0px" Style="width:100px;height:70px;"/></a>
								</center>
								</g:if>
							</td>
